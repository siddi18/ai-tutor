const ProfileDisplay = ({ profileData, onEdit, loading }) => {
  ...
  return (
    <div className="min-h-screen bg-white px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Profile Card */}
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 shadow-lg border border-emerald-100">
          {/* Profile Picture and Basic Info */}
          <div className="flex flex-col sm:flex-row items-center sm:items-start sm:space-x-6 mb-4 sm:mb-6 md:mb-8 text-center sm:text-left">
            ...
            <div>
              ...
              <p className="text-emerald-600 font-medium text-xs sm:text-sm md:text-base">
                {profileData.grade} • {profileData.stream}
              </p>
              <p className="text-gray-600 text-xs sm:text-sm md:text-base">{profileData.email}</p>
              {profileData.mobileNumber && (
                <p className="text-gray-600 text-xs sm:text-sm md:text-base">📱 {profileData.mobileNumber}</p>
              )}
            </div>
          </div>
          ...
          {profileData.aboutMe && (
            <div className="mb-4 sm:mb-6">
              <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2">About Me</h4>
              <p className="text-gray-700 bg-white p-3 sm:p-4 rounded-lg border border-emerald-100 text-xs sm:text-sm md:text-base">
                {profileData.aboutMe}
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
            <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg border border-emerald-100">
              <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">🎓 Academic Details</h4>
            ...
            <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg border border-emerald-100">
              <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">⏰ Study Schedule</h4>
              <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm md:text-base">
          ...
          {profileData.subjects && profileData.subjects.length > 0 && (
            <div className="mb-4 sm:mb-6">
              <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">📚 Subjects</h4>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {profileData.subjects.map((subject, index) => (
                  <span
                    key={index}
                    className="bg-emerald-100 text-emerald-800 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm font-medium"
                  >
                    {subject}
                  </span>
                ))}
          ...
          <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg border border-emerald-100 mb-4 sm:mb-6">
            <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">📊 Study Progress Tracker</h4>
            <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 text-center">
              <div className="bg-emerald-50 p-2 sm:p-3 rounded-lg">
                <div className="text-lg sm:text-xl md:text-2xl font-bold text-emerald-600">{calculateStudyHours().toFixed(1)}</div>
                <div className="text-xs sm:text-sm text-gray-600">Daily Hours</div>
              </div>
              <div className="bg-teal-50 p-2 sm:p-3 rounded-lg">
                <div className="text-lg sm:text-xl md:text-2xl font-bold text-teal-600">{profileData.subjects ? profileData.subjects.length : 0}</div>
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white p-3 sm:p-4 rounded-lg text-center mb-4 sm:mb-6">
            <p className="text-sm sm:text-base md:text-lg font-semibold">🌟 Ready to achieve your dreams!</p>
