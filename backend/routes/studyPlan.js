...
router.get("/latest/:userId", async (req, res) => {
  try {
    ...
    if (!studyPlan) {
      console.log(`⚠️ No study plan found for user ${userId}`);
      return res.status(404).json({ 
        error: "Study plan not found for this user",
        message: "Please create a study plan first by uploading a syllabus"
      });
    }
...
router.get("/:userId/:syllabusId", async (req, res) => {
  try {
    ...
    if (!studyPlan) {
      console.log(`⚠️ No study plan found for userId=${userId} and syllabusId=${syllabusId}`);
      return res.status(404).json({ 
        error: "Study plan not found",
        message: "No study plan found for this user and syllabus"
      });
    }
